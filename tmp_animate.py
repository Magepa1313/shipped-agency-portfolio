...
# 1. Insert Custom Cursor DOM
cursor_dom = '''
  <div class="cursor-dot" id="cursor-dot"></div>
  <div class="cursor-outline" id="cursor-outline"></div>
'''
if '<div class="cursor-dot"' not in text:
...
'''
if 'ADVANCED ANIMATIONS' not in text:
    text = text.replace('</style>', f'{style_block}\n  </style>')
...
'''
if 'Custom Cursor & Hero Parallax' not in text:
    text = text.replace('    // ── 3D Card Tilt', f'{js_block}\n    // ── 3D Card Tilt')

with open(file_path, 'w', encoding='utf-8') as f:
    f.write(text)

print('Injected advanced animations successfully.')
